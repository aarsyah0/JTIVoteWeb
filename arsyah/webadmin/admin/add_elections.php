...
<div class="row my-3">
    <div class="col-4">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                <input type="number" name="maxvote" placeholder="Max Vote" class="form-control" required />
            </div>
            <div class="form-group">
...
if(isset($_POST['addElectionBtn']))
{
    ...
    $maxvote = mysqli_real_escape_string($db, $_POST['maxvote']); // ubah sesuai dengan nama yang sesuai pada formulir
    ...
    if ($current_date >= $starting_date && $current_date <= $ending_date) {
        // Check if maxvote condition is met for Active status
        $vote_count = getVoteCount($db, $maxvote); // Ganti dengan $election_id
        if ($vote_count < $maxvote) {
            $status = "Active";
        }
    }

    // Inserting into db
    mysqli_query($db, "INSERT INTO elections(election_topic, no_of_candidates, starting_date, ending_date, maxvote, status, inserted_by, inserted_on) VALUES('". $election_topic ."', '". $number_of_candidates ."', '". $starting_date ."', '". $ending_date ."', '". $maxvote ."', '". $status ."', '". $inserted_by ."', '". $inserted_on ."')") or die(mysqli_error($db));
...
// Function to get the vote count for a specific election from multiple users
